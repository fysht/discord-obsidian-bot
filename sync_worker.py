...
def get_drive_service():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            logging.error(
                "有効なトークンがありません。generate_token.pyを実行してください。"
            )
            return None
...
def process_pending_memos():
    ...
    try:
        with lock:
            ...
            daily_notes_folder_id = find_file_in_folder(
                service,
            ...
            if not daily_notes_folder_id:
                ...
                daily_notes_folder_id = create_folder(
                    service, GOOGLE_DRIVE_FOLDER_ID, "DailyNotes"
                )
                if not daily_notes_folder_id:
                    return False
            ...
            try:
                sorted_memos = sorted(
                    [m for m in memos if m.get("id")], key=lambda m: int(m["id"])
                )
            except Exception:
            ...
            for date_str, memos_in_date in memos_by_date.items():
                ...
                for memo in memos_in_date:
                    ...
                    try:
                        ts = datetime.fromisoformat(
                            memo["created_at"].replace("Z", "+00:00")
                        ).astimezone(JST)
                ...
                if file_id:
                    ...
                    new_id = create_text_file(
                        service, daily_notes_folder_id, file_name, full_new_content
                    )
                    if new_id:
                        success = True
            ...
            if not failed_ids:
                ...
                if latest_processed_id:
                    bot_folder_id = find_file_in_folder(
                        service,
                        GOOGLE_DRIVE_FOLDER_ID,
                        ".bot",
                        "application/vnd.google-apps.folder",
                    ...
                    if not bot_folder_id:
                        bot_folder_id = create_folder(
                            service, GOOGLE_DRIVE_FOLDER_ID, ".bot"
                        )

                    if bot_folder_id:
                        last_id_file_id = find_file_in_folder(
                            service, bot_folder_id, "last_processed_id.txt"
                        )
                        if last_id_file_id:
                            update_text_file(
                                service, last_id_file_id, str(latest_processed_id)
                            )
                        else:
                            create_text_file(
                                service,
                ...
                remaining_memos = [
                    m
                    for m in memos
                    if m.get("id") in failed_ids or m.get("id") not in processed_ids
                ]
