...
class FitbitService:
    ...
    async def _load_token(self):
        ...
        try:
            f_id = await self.drive_service.find_file(
                service, self.drive_service.folder_id, TOKEN_FILE_NAME
            )
            if f_id:
        ...
        try:
            ...
            if f_id:
                ...
                await self.drive_service.upload_text(
                    service,
                    self.drive_service.folder_id,
                    TOKEN_FILE_NAME,
                    new_refresh_token,
                )
            logging.info(
                "🎉 新しいFitbitリフレッシュトークンをGoogle Driveに保存しました。"
            )
        except Exception as e:
    ...
    def _calculate_sleep_score(
        ...
        restlessness_percentage = (
            (wake_min / total_in_bed_min) * 100 if total_in_bed_min > 0 else 100
        )
        restoration_score = (
            25
            if restlessness_percentage <= 5
            else 22
            if restlessness_percentage <= 10
            else 18
            if restlessness_percentage <= 15
            else 14
            if restlessness_percentage <= 20
            else 10
        )
                ...
                if resp.status == 200:
                    ...
                    if isinstance(raw_hr_zones, list):
                        for z in raw_hr_zones:
                            ...
                            if name == "Fat Burn":
                                stats["hr_zone_fat_burn_minutes"] = mins
                            elif name == "Cardio":
                                stats["hr_zone_cardio_minutes"] = mins
                            elif name == "Peak":
                                stats["hr_zone_peak_minutes"] = mins
                else:
                    error_text = await resp.text()
                    logging.error(
                        f"[Fitbit Activity Error] Status: {resp.status}, Body: {error_text}"
                    )
        except Exception as e:
        ...
        try:
            ...
            async with session.get(url, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    if sleep_log:
                        main_sleep = max(
                            sleep_log, key=lambda x: x.get("minutesAsleep", 0)
                        )
                        ...
                        stats["sleep_score"] = self._calculate_sleep_score(
                            total_asleep, total_in_bed, deep_min, rem_min, wake_min
                        )
                else:
                    error_text = await resp.text()
                    logging.error(
                        f"[Fitbit Sleep Error] Status: {resp.status}, Body: {error_text}"
                    )
        except Exception as e:
    ...
    def _update_note_content(self, content, stats):
        ...
        update_keys = [
            ...
            "resting_heart_rate",
            "active_minutes_very",
            "active_minutes_fairly",
            "active_minutes_lightly",
            "sedentary_minutes",
            "hr_zone_fat_burn_minutes",
            "hr_zone_cardio_minutes",
            "hr_zone_peak_minutes",
        ]
        ...
        if "steps" in stats:
            ...
            if any(
                k in stats for k in ["resting_heart_rate", "hr_zone_fat_burn_minutes"]
            ):
                heart_rate_text = (
                    f"#### ❤️ Heart Rate\n"
