...
NEWS_BRIEFING_TIME = time(hour=8, minute=35, tzinfo=JST)
...
class NewsCog(commands.Cog):
    ...
    async def _get_weather_forecast(self, coords: dict, location_name: str) -> str:
        try:
            # Freeプラン対応: one_call ではなく weather_at_coords を使用
            observation = await asyncio.to_thread(
                self.mgr.weather_at_coords, coords['lat'], coords['lon']
            )
            weather = observation.weather
            temp = weather.temperature('celsius')
            status = weather.detailed_status
            return f"**{location_name}**: {status} | 気温 {temp['temp']:.0f}℃ (最高 {temp.get('max','?')}℃ / 最低 {temp.get('min','?')}℃)"
        except Exception as e:
    ...
    async def daily_news_briefing(self):
        ...
        # --- マクロ経済ニュースを投稿 ---
        target_sites = [
            "site:nikkei.com", "site:toyokeizai.net", "site:jp.reuters.com", "site:bloomberg.co.jp"
        ]
        sites_query = " OR ".join(target_sites)
        
        # 毎日注目すべき大きな経済ニュースを拾う
        market_queries = [f"({sites_query}) (日銀 OR 金融政策 OR 経済)"]
