        ...
        try:
            ...
            try:
                transcript_list = await asyncio.to_thread(
                    YouTubeTranscriptApi.get_transcript, video_id, ['ja', 'en']
                )
                # ✅ 修正版: item.text でアクセス
                transcript_text = " ".join(
                    [item.text if hasattr(item, "text") else item["text"] for item in transcript_list]
                )
            ...
            with dropbox.Dropbox(
                ...
                if youtube_heading in daily_note_content:
                    daily_note_content = daily_note_content.replace(
                        youtube_heading, f"{youtube_heading}\n{link_to_add}"
                    )
                else:
