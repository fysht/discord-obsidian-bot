...
class AddToListView(discord.ui.View):
        ...
        if self.reply_message:
            try:
                ...
                await self.reply_message.delete()
                 ...
                 logging.error(f"Failed to edit/delete reply message: {e}")
        self.stop()
    ...
    async def cancel_and_select(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.reply_message:
             ...
             except discord.HTTPException as e: logging.warning(f"Failed to edit reply message: {e}")
    ...
    async def memo_only(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.reply_message:
            try:
                ...
                await asyncio.sleep(10)
                await self.reply_message.delete()
            except discord.HTTPException as e: logging.error(f"Failed to edit/delete reply message (memo_only): {e}")
        self.stop()
    ...
    async def on_timeout(self):
        ...
        if self.reply_message:
            try:
                ...
                await asyncio.sleep(10)
                await self.reply_message.delete()
            except discord.HTTPException as e: logging.warning(f"Failed to edit/delete reply message on timeout: {e}")
        self.stop()
...
class AddItemModal(discord.ui.Modal, title="リストに項目を追加"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if context not in ["Work", "Personal"]: await interaction.followup.send("⚠️ コンテキストは 'Work' または 'Personal' で入力してください。", ephemeral=True); return
        if category not in CATEGORY_MAP: await interaction.followup.send(f"⚠️ カテゴリは {', '.join(CATEGORY_MAP.keys())} のいずれかで入力してください。", ephemeral=True); return
        if not item: await interaction.followup.send("⚠️ 追加する項目名を入力してください。", ephemeral=True); return
...
class CompleteItemModal(discord.ui.Modal, title="リストの項目を完了"):
    def __init__(self, memo_cog_instance, items_by_category: dict):
        ...
        self.category_select = discord.ui.Select(placeholder=placeholder_text, options=options if options else [discord.SelectOption(label="項目なし", value="no_items")], custom_id="category_select", disabled=not options)
    ...
    async def on_category_select(self, interaction: discord.Interaction):
        if not interaction.data or 'values' not in interaction.data or not interaction.data['values']: logging.warning("on_category_select: No values selected."); await interaction.response.send_message("カテゴリが選択されていません。", ephemeral=True, delete_after=5); return
        ...
        if items:
            ...
            self.item_select.options = options_for_select; self.item_select.placeholder = "完了する項目を選択"; self.item_select.disabled = False
        else: self.item_select.options = []; self.item_select.placeholder = "このリストに項目はありません"; self.item_select.disabled = True
        try: await interaction.edit_original_response(view=self)
        except discord.HTTPException as e: logging.error(f"Failed to edit modal view: {e}")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        else: await interaction.followup.send("❌ 完了処理中にエラーが発生しました。", ephemeral=True)

class AddToCalendarModal(discord.ui.Modal, title="カレンダーに登録"):
    def __init__(self, memo_cog_instance, tasks: list):
        super().__init__(timeout=None)
        self.memo_cog = memo_cog_instance
        if tasks: options = [discord.SelectOption(label=task[:100], value=task) for task in tasks][:25]; placeholder = "カレンダーに登録するタスクを選択..."; disabled = False
        else: options = [discord.SelectOption(label="タスクなし", value="no_task")]; placeholder = "登録できるタスクがありません"; disabled = True
        self.task_select = discord.ui.Select(placeholder=placeholder, options=options, disabled=disabled)
        self.add_item(self.task_select)
        self.target_date = discord.ui.TextInput(label="日付 (YYYY-MM-DD形式、空欄で今日)", required=False, placeholder=datetime.now(JST).strftime('%Y-%m-%d'))
        self.add_item(self.target_date)

    async def on_submit(self, interaction: discord.Interaction):
        if not self.task_select.values or self.task_select.values[0] == "no_task": await interaction.response.send_message("タスクが選択されていません。", ephemeral=True, delete_after=10); return
        await interaction.response.defer(ephemeral=True, thinking=True)
        selected_task = self.task_select.values[0]
        target_date_str = self.target_date.value.strip()
        target_date_obj = datetime.now(JST).date()
        if target_date_str:
            try: target_date_obj = datetime.strptime(target_date_str, '%Y-%m-%d').date()
            except ValueError: await interaction.followup.send("⚠️ 日付の形式が正しくありません。", ephemeral=True); return
        journal_cog = self.memo_cog.bot.get_cog('JournalCog')
        if not journal_cog or not journal_cog.is_ready or not journal_cog.calendar_service: await interaction.followup.send("⚠️ カレンダー機能が利用できません。", ephemeral=True); return
        try:
            ...
            await interaction.followup.send(f"✅ 「{selected_task}」を {target_date_obj.strftime('%Y-%m-%d')} の終日予定として登録しました。", ephemeral=True)
        except Exception as e: logging.error(f"カレンダー登録中にエラー ({selected_task}): {e}", exc_info=True); await interaction.followup.send(f"❌ カレンダー登録処理中にエラーが発生: `{e}`", ephemeral=True)
...
class MemoCog(commands.Cog):
    def __init__(self, bot):
        ...
        try:
            ...
            logging.info("MemoCog: Dropbox client initialized and verified successfully.")
            self.dbx_available = True
        except Exception as e:
             logging.error(f"MemoCog: Failed to initialize or verify Dropbox client: {e}")
             self.dbx = None; self.dbx_available = False

        if self.gemini_api_key:
            try:
                genai.configure(api_key=self.gemini_api_key)
                self.gemini_model = genai.GenerativeModel("gemini-2.5-pro")
                logging.info("MemoCog: Gemini model initialized successfully.")
                self.gemini_available = True
            except Exception as e:
                 logging.error(f"MemoCog: Failed to initialize Gemini model: {e}")
                 self.gemini_model = None; self.gemini_available = False
        else:
            self.gemini_model = None; self.gemini_available = False
            logging.warning("MemoCog: GEMINI_API_KEY not set. AI categorization disabled.")

        if LIST_CHANNEL_ID != 0 and self.dbx_available:
            self.post_all_lists.start()
        elif LIST_CHANNEL_ID != 0:
             logging.warning("MemoCog: Dropbox client not available. List posting task disabled.")

    def cog_unload(self):
        if hasattr(self, 'post_all_lists') and self.post_all_lists.is_running():
    ...
    def _sort_tasks_with_deadline(self, tasks: list) -> list:
        def get_deadline(task_text):
            ...
            if match:
                try:
                    month, day = map(int, match.groups()); today = datetime.now(JST); year = today.year
                    deadline_date = datetime(year, month, day)
                    if deadline_date.date() < today.date(): year += 1
    ...
    async def get_list_items(self, category: str, context: str) -> list[str]:
        ...
        try:
            ...
            content = res.content.decode('utf-8'); items = re.findall(r"-\s*\[\s*\]\s*(.+)", content)
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            logging.info(f"Text memo saved (ID: {message.id})")
            if self.gemini_available: await self.categorize_and_propose_action(message)
            else: logging.info(f"Skipping AI categorization (Gemini not available) for message {message.id}")
        except Exception as e:
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        try:
            ...
            content = message.content.strip()
            if message.author.bot or not content: # 空メッセージは対象外
            ...
            if self.gemini_available: await self.categorize_and_propose_action(message)
            else: await channel.send("⚠️ AI分類機能は現在利用できません。", delete_after=10)
            user = await self.bot.fetch_user(payload.user_id)
            if user: await message.remove_reaction(payload.emoji, user)
    ...
    async def categorize_and_propose_action(self, message: discord.Message):
        ...
        try:
            ...
            if not json_match: logging.warning(f"No JSON found in classification response: {response.text}"); return
            result_json = json.loads(json_match.group(1))
            context = result_json.get("context"); category = result_json.get("category"); item = result_json.get("item", "").strip()
            if context in ["Work", "Personal"] and category in CATEGORY_MAP:
                if not item and category != "Other": logging.warning(f"AI classification invalid item for '{category}': {message.content}"); return
                if category != "Other":
        ...
        except json.JSONDecodeError as e: logging.warning(f"JSON parsing failed for classification: {e}\nResponse: {getattr(response, 'text', 'N/A')}")
    ...
    async def post_all_lists(self):
        if not self.dbx_available: logging.warning("post_all_lists: Dropbox not available."); return
        await self.refresh_all_lists_post()

    async def refresh_all_lists_post(self, channel=None):
        if not self.dbx_available: logging.warning("refresh_all_lists_post: Dropbox not available."); return
        ...
        if not all_items_structured and self.dbx_available: embed.description = "リスト情報取得失敗"; logging.error("Failed to get structured list items.")
            ...
            for context_value, categories in all_items_structured.items():
                ...
                for cat_choice in CATEGORY_CHOICES:
                    category_value = cat_choice.value; category_name = cat_choice.name
                    items = categories.get(category_value, [])
                ...
                if field_value: embed.add_field(name=f"--- {context_name} ---", value=field_value.strip() if field_value else "項目なし", inline=False)
        ...
        try:
            ...
            if self.last_list_message_id and last_message and last_message.id == self.last_list_message_id:
                ...
                await message.edit(embed=embed, view=view); logging.info(f"リスト投稿編集 (ID: {self.last_list_message_id})")
            else:
                ...
                if deleted_count > 0: logging.info(f"{deleted_count}件の古いリスト投稿削除")
                message = await channel.send(embed=embed, view=view); self.last_list_message_id = message.id
                logging.info(f"リスト新規投稿 (ID: {self.last_list_message_id})")
        except (discord.NotFound, discord.Forbidden) as e:
            logging.warning(f"リスト投稿編集/削除失敗 ({e})。新規投稿試行。")
            try:
                # 古いメッセージ削除再試行
                ...
                message = await channel.send(embed=embed, view=view); self.last_list_message_id = message.id
                logging.info(f"リスト新規投稿 (ID: {self.last_list_message_id})")
            except discord.HTTPException as send_e: logging.error(f"リスト新規投稿失敗: {send_e}"); self.last_list_message_id = None
        except Exception as e: logging.error(f"リスト投稿更新エラー: {e}", exc_info=True); self.last_list_message_id = None
