        ...
        with lock:
            ...
            if PENDING_MEMOS_FILE.exists() and PENDING_MEMOS_FILE.stat().st_size > 0:
                try:
                    ...
                    logging.warning(f"[obsidian_handler] JSON decode error in {PENDING_MEMOS_FILE}. Starting fresh.")
...
async def add_memo_async(content, *, author="Unknown", created_at=None, message_id=None, context=None, category=None):
    ...
    await loop.run_in_executor(None, _add_memo_sync, content, author, created_at_iso, message_id, context, category)